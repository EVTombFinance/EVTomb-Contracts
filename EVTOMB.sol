 ...
 * functions have evtombn added to mitigate the well-known issues around setting
...
// File contracts/EVTomb.sol


pragma solidity 0.6.12;
contract EVTomb is ERC20Burnable, Operator {
    ...
    // Have the rewards evtombn distributed to the pools
    ...
    /**
     * @notice Constructs the EVTOMB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("evtomb", "EVTOMB") {
        // Mints 1 EVTOMB to contract creator for initial pool setup
    ...
    function _getEVTombPrice() internal view returns (uint256 _evtombPrice) {
        try IOracle(evtombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("EVTomb: failed to fetch EVTOMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _evtombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_evtombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setEVTombOracle(address _evtombOracle) public onlyOperatorOrTaxOffice {
        require(_evtombOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints EVTOMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of EVTOMB to mint to
     * @return whether the process has evtombn done
     */
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEVTombPrice = _getEVTombPrice();
            currentTaxRate = _updateTaxRate(currentEVTombPrice);
            if (currentEVTombPrice < burnThreshold) {
                burnTax = true;
            }
        }
